...
Estado atualizado: backend verificado localmente e CI 33577099220 aprovado para backend, Flutter e Compose. Ver os checks do PR #2 para o commit mais recente. Base: Phase 0 e correções do contrato no commit db83f89a79c47c9f68ca365aac3280b5436b7b89.
...
Baseline explícita, não declaração de versões mais recentes: Node 22.23.2, pnpm 10.11.0, Nest 11.2.3, Prisma 6.19.3, Flutter 3.35.7. As versões diretas dos packages estão fixadas. Prisma 6 foi escolhido para o bootstrap com client generator e configuração no schema; upgrades de major exigem testes. Referências: [Prisma 6.19](https://www.prisma.io/blog/announcing-prisma-6-19-0), [arquivo Flutter](https://docs.flutter.dev/install/archive).

Os lockfiles pnpm-lock.yaml e pubspec.lock estão versionados a partir de resoluções reais. CI/Docker usam --frozen-lockfile; Flutter usa --enforce-lockfile. Formatação backend é gate do CI. Docker/Actions ainda devem ser fixados por digest/SHA antes de release de produção.

A auditoria da resolução corrigida não reporta findings conhecidos. Ver phase-1-stabilization.md para versões e justificação dos overrides.
...
- Os comandos foram verificados nos ambientes descritos em phase-1-stabilization.md; CI continua a ser obrigatório em cada alteração.
