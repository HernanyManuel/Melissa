...
test('automatic outbound queue accepts only minimal valid jobs', () => {
  const id = randomUUID();
  assert.equal(isAIAutomaticOutboundJob('ai-automatic-outbound', { id, attempt: 0 }), true);
  assert.equal(isAIAutomaticOutboundJob('ai-automatic-outbound', { id, attempt: 4 }), true);
  assert.equal(
  ...
  assert.equal(isAIAutomaticOutboundJob('ai-automatic-outbound', { id, attempt: 5 }), false);
  assert.equal(
